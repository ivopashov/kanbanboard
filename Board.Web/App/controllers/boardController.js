    ...
    function ($scope, boardService, notificationService, $modal, stringManipulationService) {
        ...
        boardService.allStatuses().then(function (statuses) {
            boardService.getAll().then(function (cards) {
                angular.forEach(statuses.data, function (val) {
                    ...
                    if (cards.data.hasOwnProperty(statusLowerCaseFirstLetter)) {
                        $scope.allCards[statusLowerCaseFirstLetter] = cards.data[statusLowerCaseFirstLetter];
                    } else {
